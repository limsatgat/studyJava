...
public class SwingCalendarSample2 extends JFrame implements ItemListener, ActionListener {
...
//        int lastDay = date.getMaximum(Calendar.DATE); //getMaximum 는 Calender 자체가 최대로 가질수 있는 값 - > 모든 월의 마지막날이 31일로 출력됨
    ...
    public void setCalendarTitle() { //메소드
        for (int i = 0; i < title.length; i++) { //만들어준 배열의 수만큼 돌린다.
            ...
            if (i == 0) lbl.setForeground(Color.red); //setForeground 폰트 속성을 변경해주는데 쓰는것,
